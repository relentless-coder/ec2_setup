...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | sudo apt-key add -
            ...
            sudo apt-get update && sudo apt-get install yarn && echo "Node version installed is $(yarn -v)"; break;;
        No ) break;;
    esac
done
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) sudo apt-get install build-essential tcl
            ...
            sudo addgroup redis
            sudo adduser --system redis --ingroup redis
            sudo cat $dir/redis.systemd.txt >> /etc/systemd/system/redis.service
            sudo systemctl start redis 
            sudo systemctl enable redis;  break;;
        No ) break;;
    esac
done
